import React from 'react';
import { Mail, Instagram, Linkedin, Facebook } from 'lucide-react';

const LinktreeIcon = () => (
  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M13.5108 5.85343L17.5158 1.73642L19.8404 4.11701L15.6393 8.12199H21.5488V11.4268H15.6113L19.8404 15.5345L17.5158 17.8684L11.7744 12.099L6.03299 17.8684L3.70842 15.5438L7.93745 11.4361H2V8.12199H7.90944L3.70842 4.11701L6.03299 1.73642L10.038 5.85343V0H13.5108V5.85343ZM10.038 16.16H13.5108V24H10.038V16.16Z" />
  </svg>
);

export function Footer() {
  return (
    <footer className="bg-gray-900 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Contact Us Section */}
        <div className="text-center mb-8">
          <h3 className="text-white mb-6">Contact Us</h3>
          <div className="flex justify-center gap-4">
            <a
              href="https://www.instagram.com/mpma.mcgill/"
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 bg-gray-800 rounded-lg hover:bg-gradient-to-r hover:from-[#ED1B2F] hover:to-[#2563EB] transition-all"
            >
              <Instagram className="w-5 h-5" />
            </a>
            <a
              href="https://www.linkedin.com/company/mpma-mcgill/"
              className="p-2 bg-gray-800 rounded-lg hover:bg-gradient-to-r hover:from-[#ED1B2F] hover:to-[#2563EB] transition-all"
            >
              <Linkedin className="w-5 h-5" />
            </a>
            <a
              href="https://www.facebook.com/mpma.mcgill/"
              className="p-2 bg-gray-800 rounded-lg hover:bg-gradient-to-r hover:from-[#ED1B2F] hover:to-[#2563EB] transition-all"
            >
              <Facebook className="w-5 h-5" />
            </a>
            <a
              href="https://linktr.ee/mpma_mcgill"
              className="p-2 bg-gray-800 rounded-lg hover:bg-gradient-to-r hover:from-[#ED1B2F] hover:to-[#2563EB] transition-all"
            >
              <LinktreeIcon />
            </a>
            <a
              href="mailto:mpma.comms@mcgilleus.ca"
              className="p-2 bg-gray-800 rounded-lg hover:bg-gradient-to-r hover:from-[#ED1B2F] hover:to-[#2563EB] transition-all"
            >
              <Mail className="w-5 h-5" />
            </a>
          </div>
        </div>

        {/* Copyright */}
        <div className="pt-8 border-t border-gray-800 text-center">
          <p className="text-gray-400">
            © 2025 McGill Product Management Association
          </p>
        </div>
      </div>
    </footer>
  );
}
